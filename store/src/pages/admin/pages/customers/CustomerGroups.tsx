...
const CustomerGroups: React.FC = () => {
  ...
  const [newGroupData, setNewGroupData] = useState<Omit<Group, 'id'>>({name: "", status: ""});

  const fetchGroups = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await getAllGroups(authToken as string, currentPage);
      if (response.success) {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault();
    // Here you would typically send the new group data to your API
    // For this example, we'll just log it to the console
    try {
      const newgroupId = await createCustomerGroup(authToken as string, newGroupData);
      if (newgroupId) {
        setGroups((prevGroups) => [
          ...prevGroups,
          { id: newgroupId, ...newGroupData },
        ]);
        ...
        {loading ?
          ...
          : groups.length === 0 ? (
            <EmptyState
              ...
              description="Try adjusting your search or filter criteria, or add a new Category."
              action={{
                label: "Reset Filters",
                onClick: () => {
                  setSearchTerm("");
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
          ...
          <div className="flex space-x-4">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md ${statusFilter === "pending"
                ? "bg-blue-600 text-white"
                : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                }`}
            >
